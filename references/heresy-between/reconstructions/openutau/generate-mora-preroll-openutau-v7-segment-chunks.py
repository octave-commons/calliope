#!/usr/bin/env python3
"""Generate v7 OpenUTAU chunks that never split a lyric segment at chunk edges.

v6 fixed lyric cycling but retained old fixed chunk boundaries; several boundaries
cut through a phrase, so chunks 02/03 began mid-word/mid-line. v7 keeps the same
one-note-per-mora policy and moves chunk boundaries to transcript segment starts.
"""
...
OUT_DIR = OUT_BASE / 'v7-segment-boundary-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-segment-boundary-openutau-v7.json'
...
def ustx_text(chunk: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-segment-boundary-openutau-v7-{chunk['id']}
comment: 'v7 segment-boundary reconstruction: one note per mora, no cyclic lyric reuse, chunk edges on transcript segment boundaries.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu segment-boundary vocal v7 {chunk['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu segment-boundary vocal v7 {chunk['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for chunk in CHUNKS:
        ...
        ustx_path = OUT_DIR / f"heresy-between-segment-boundary-openutau-v7-{chunk['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'segment-boundary OpenUTAU v7 full chunks',
        ...
        'note_strategy': 'One sung note per kana mora. Chunk boundaries are transcript segment edges so no chunk starts mid-phrase. Extra pitch events do not create extra lyrics.',
    }
