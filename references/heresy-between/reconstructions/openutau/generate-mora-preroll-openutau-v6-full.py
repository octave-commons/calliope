#!/usr/bin/env python3
"""Generate full-song v6 USTX chunks that prioritize correct lyrics.

This is the durable version of the opening v6 probe:
- one sung note per kana mora;
- never cycle/repeat lyrics to fill pitch events;
- preserve segment start/end timing from the hybrid transcript layer;
- add render pre-roll to every chunk so first phonemes have context.
"""
...
OUT_DIR = OUT_BASE / 'v6-mora-preroll-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-mora-preroll-openutau-v6.json'
TICKS_PER_SECOND = 800
PREROLL_SECONDS = 0.5
TARGET_DURATION = 303.32
CHUNKS = [
    {'id': 'chunk-01', 'start': 0.0, 'end': 58.0},
    {'id': 'chunk-02', 'start': 58.0, 'end': 106.0},
    {'id': 'chunk-03', 'start': 106.0, 'end': 174.0},
    {'id': 'chunk-04', 'start': 174.0, 'end': 244.0},
    {'id': 'chunk-05', 'start': 244.0, 'end': 303.32},
]
...
def ustx_text(chunk: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-mora-preroll-openutau-v6-{chunk['id']}
comment: 'v6 lyric-first reconstruction: one note per mora, no cyclic lyric reuse, render pre-roll.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu mora-preroll vocal v6 {chunk['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu mora-preroll vocal v6 {chunk['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for segment_index, segment in enumerate(hybrid['segments']):
        ...
        segment_readings.append(reading)
    ...
    for chunk in CHUNKS:
        ...
        ustx_path = OUT_DIR / f"heresy-between-mora-preroll-openutau-v6-{chunk['id']}.ustx"
        if ustx_path.exists():
        ...
        chunks.append({
            **chunk,
            ...
            'first_lyrics': [n['lyric'] for n in notes[:24]],
        })
    manifest = {
        'generated_at': dt.datetime.now(dt.UTC).isoformat(),
        'kind': 'mora-preroll OpenUTAU v6 full chunks',
        ...
        'note_strategy': 'One sung note per kana mora from best_reference_match/text. Never reuse/cycle segment lyrics when there are more pitch events than morae; pitch events only estimate tone. Chunk renders include pre-roll and should be assembled with assembly_offset_seconds.',
    }
    ...
    print(json.dumps({
        ...
        'out_dir': str(OUT_DIR),
        'total_note_count': len(segment_notes),
        'chunks': [{'id': c['id'], 'note_count': c['note_count'], 'ustx_path': c['ustx_path']} for c in chunks],
        'opening_first_lyrics': chunks[0]['first_lyrics'] if chunks else [],
