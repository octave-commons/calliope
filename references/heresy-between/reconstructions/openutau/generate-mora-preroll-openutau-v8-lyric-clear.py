#!/usr/bin/env python3
"""Generate v8 lyric-clear OpenUTAU chunks with segment-safe edges and stable pitches.

v8 keeps the v7 segment-boundary fix but uses stable, low-variance pitches for
word-recognition reference renders. This branch prioritizes lyric intelligibility
over matching the source melody.
"""
...
OUT_DIR = OUT_BASE / 'v8-lyric-clear-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-lyric-clear-openutau-v8.json'
...
def pitch_for_interval(events: list[dict], start_s: float, end_s: float, fallback: int) -> int:
    # Lyric-clear branch: avoid source melody jumps that make Ritsu's consonants
    # collapse under ASR/Gemma. A stable alto register kept controlled tests
    # recognizable even at the original mora speed.
    return 60
...
def ustx_text(chunk: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-lyric-clear-openutau-v8-{chunk['id']}
comment: 'v8 lyric-clear reconstruction: one note per mora, stable pitch, no cyclic lyric reuse, chunk edges on transcript segment boundaries.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu lyric-clear vocal v8 {chunk['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu lyric-clear vocal v8 {chunk['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for chunk in CHUNKS:
        ...
        ustx_path = OUT_DIR / f"heresy-between-lyric-clear-openutau-v8-{chunk['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'lyric-clear OpenUTAU v8 full chunks',
        ...
        'note_strategy': 'One sung note per kana mora. Chunk boundaries are transcript segment edges so no chunk starts mid-phrase. Stable pitch prioritizes word intelligibility; extra pitch events do not create extra lyrics.',
    }
