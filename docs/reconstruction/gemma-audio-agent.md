...
`metrics` uses `scripts/reconstruction/audio_metrics.py` through the local MIR workbench venv at `/home/err/Music/fork-tales/references/mir-workbench/.venv/bin/python`. It writes JSON plus visual artifacts:
