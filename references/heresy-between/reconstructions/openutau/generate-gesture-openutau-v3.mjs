...
const chunksDir = `${outBase}/v3-gesture-chunks`;
const gesturePath = `${base}/lyrics/auditions/gesture-layer-v2-dense-legato.json`;
const manifestPath = `${outBase}/heresy-between-gesture-openutau-v3.json`;
...
function ustxForChunk(chunk, notes) {
  ...
  return `name: heresy-between-gesture-openutau-v3-${chunk.id}
comment: 'Gesture-derived OpenUTAU audition v3 from isolated vocal ASR + f0. Not prompt-lyrics branch.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu gesture vocal v3 ${chunk.id}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: ${duration}
  name: Ritsu gesture vocal v3 ${chunk.id}
  track_no: 0
  position: 0
  notes:
${yamlNotes(notes)}
...
for (const chunk of chunks) {
  ...
  const ustxPath = `${chunksDir}/heresy-between-gesture-openutau-v3-${chunk.id}.ustx`;
...
const manifest = {
  ...
  kind: 'Gesture-derived OpenUTAU audition v3',
  ...
  note_strategy: 'Use gesture-layer syllable-ish events and f0 tones; kana is sourced from ASR segment text when available, otherwise safe kana pool. This is an audition of timing/gesture through Ritsu, not final lyrics.',
};
