#!/usr/bin/env python3
"""Generate v13 tail-context OpenUTAU render units.

v13 keeps v12 render units but adds real following-note tail context for the
世界 phrase unit. Phrase probes showed that rendering segment 21-22 plus the
next short phrase, then trimming back to the original span, fixes the local STT
世界/正義 ambiguity.
"""
...
OUT_DIR = OUT_BASE / 'v13-tail-context-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-tail-context-openutau-v13.json'
...
RENDER_UNITS = [
    ...
    {
        'id': 'unit-04-sekai-phrase-with-tail-context',
        'render_start': 103.64,
        'render_end': 118.0,
        'assembly_start': 103.64,
        'assembly_end': 113.86,
        'render_tail_seconds': 2.0,
        'policy': 'phrase-isolated-render-with-segment-23-tail-context-fixes-sekai',
    },
    {
...
def ustx_text(unit: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-tail-context-openutau-v13-{unit['id']}
comment: 'v13 tail-context reconstruction: generated phrase-isolated units with exact assembly trims and render-tail policy.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu tail-context vocal v13 {unit['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu tail-context vocal v13 {unit['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for unit in RENDER_UNITS:
        ...
        if unit.get('kind') == 'silence':
            manifest_units.append({
                **unit,
                ...
                'planned_aligned_wav_path': str(OUT_DIR / f"heresy-between-tail-context-openutau-v13-{unit['id']}.wav"),
        ...
        ustx_path = OUT_DIR / f"heresy-between-tail-context-openutau-v13-{unit['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'tail-context OpenUTAU v13 full timeline',
        ...
        'note_strategy': 'v13 promotes v11/v12 audited phrase probes into generated render units with real following-note tail context: exact assembly spans, optional render context, render-tail extension for short phrases, small-tsu preservation, long-vowel preservation for segment 11, and stable pitch for segments 10-36.',
    }
