#!/usr/bin/env python3
"""Generate v16 exact phrase-grid OpenUTAU render units.

v16 keeps v15 and also reproduces the exact small-tsu-aware integer grid for
the 強がってスクロール phrase unit. Tiny 1-4 tick source-rounding drift was enough
to turn スクロール into クロール under local STT.
"""
...
OUT_DIR = OUT_BASE / 'v16-exact-phrase-grid-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-exact-phrase-grid-openutau-v16.json'
...
RENDER_UNITS = [
    ...
    },
    {
        ...
        'assembly_end': 64.82,
        'render_tail_seconds': 0.5,
        'policy': 'literal-small-tsu-and-long-vowel-source-text-segment-11-exact-grid',
        'exact_grid_segments': [11],
    },
    {
...
def ustx_text(unit: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-exact-phrase-grid-openutau-v16-{unit['id']}
comment: 'v16 exact-phrase-grid reconstruction: generated phrase-isolated units with exact assembly trims and render-tail policy.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu exact-phrase-grid vocal v16 {unit['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu exact-phrase-grid vocal v16 {unit['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for unit in RENDER_UNITS:
        ...
        if unit.get('kind') == 'silence':
            manifest_units.append({
                **unit,
                ...
                'planned_aligned_wav_path': str(OUT_DIR / f"heresy-between-exact-phrase-grid-openutau-v16-{unit['id']}.wav"),
        ...
        if unit.get('exact_grid_segments'):
            ...
            for seg_idx in unit['exact_grid_segments']:
                ...
                small_count = sum(1 for token in seg['tokens'] if token['kind'] == 'small_tsu')
                normal_count = max(1, len(seg['tokens']) - small_count)
                normal_duration = round((seg_ticks - small_count * SMALL_TSU_TICKS) / normal_count)
                for token_idx, token in enumerate(seg['tokens']):
                    token_duration = SMALL_TSU_TICKS if token['kind'] == 'small_tsu' else normal_duration
                    notes.append({
                        'position': cursor,
        ...
        ustx_path = OUT_DIR / f"heresy-between-exact-phrase-grid-openutau-v16-{unit['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'exact-phrase-grid OpenUTAU v16 full timeline',
