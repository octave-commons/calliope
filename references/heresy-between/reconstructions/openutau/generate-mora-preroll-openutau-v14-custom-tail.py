#!/usr/bin/env python3
"""Generate v14 custom-tail OpenUTAU render units.

v14 keeps v12 render units but adds the exact custom tail that passed the
世界 phrase probe: segment 21-22 followed by a synthetic それだけでいい tail,
then trimmed back to the original span.
"""
...
OUT_DIR = OUT_BASE / 'v14-custom-tail-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-custom-tail-openutau-v14.json'
...
RENDER_UNITS = [
    ...
    },
    {
        ...
        'policy': 'phrase-isolated-render-with-custom-soredakedeii-tail-fixes-sekai',
...
def ustx_text(unit: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-custom-tail-openutau-v14-{unit['id']}
comment: 'v14 custom-tail reconstruction: generated phrase-isolated units with exact assembly trims and render-tail policy.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu custom-tail vocal v14 {unit['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu custom-tail vocal v14 {unit['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for unit in RENDER_UNITS:
        ...
        if unit.get('kind') == 'silence':
            manifest_units.append({
                **unit,
                ...
                'planned_aligned_wav_path': str(OUT_DIR / f"heresy-between-custom-tail-openutau-v14-{unit['id']}.wav"),
        ...
        ustx_path = OUT_DIR / f"heresy-between-custom-tail-openutau-v14-{unit['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'custom-tail OpenUTAU v14 full timeline',
        ...
        'note_strategy': 'v14 promotes v11/v12 audited phrase probes into generated render units with custom phrase-tail context: exact assembly spans, optional render context, render-tail extension for short phrases, small-tsu preservation, long-vowel preservation for segment 11, and stable pitch for segments 10-36.',
    }
