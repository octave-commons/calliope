#!/usr/bin/env python3
"""Generate v12 render-unit OpenUTAU USTX files.

v12 promotes the v11 best-audit audio composite back into a single generated
branch: each audible timeline span has an explicit render unit, optional render
context, optional tail allowance, and exact assembly trim metadata.

This keeps the workflow reproducible without reusing ad-hoc phrase-probe WAVs.
"""
...
OUT_DIR = OUT_BASE / 'v12-render-unit-chunks'
...
MANIFEST_PATH = OUT_BASE / 'heresy-between-render-unit-openutau-v12.json'
...
RENDER_UNITS = [
    ...
    {
        'id': 'unit-04-sekai-phrase-with-tail',
        'render_start': 103.64,
        'render_end': 113.86,
        'assembly_start': 103.64,
        'assembly_end': 113.86,
        'render_tail_seconds': 2.0,
        'policy': 'phrase-isolated-render-tail-fixes-sekai',
    },
    {
...
def ustx_text(unit: dict, notes: list[dict]) -> str:
    ...
    return f"""name: heresy-between-render-unit-openutau-v12-{unit['id']}
comment: 'v12 render-unit reconstruction: generated phrase-isolated units with exact assembly trims and render-tail policy.'
output_dir: Export
cache_dir: UCache
ustx_version: '0.6'
resolution: 480
...
- singer: 波音リツ連続音Ver1.5.1
  ...
  track_name: Ritsu render-unit vocal v12 {unit['id']}
  track_color: Blue
  mute: false
  solo: false
  volume: 0
...
voice_parts:
- duration: {duration}
  name: Ritsu render-unit vocal v12 {unit['id']}
  track_no: 0
  position: 0
  notes:
{notes_text}
...
def main() -> None:
    ...
    for unit in RENDER_UNITS:
        ...
        if unit.get('kind') == 'silence':
            manifest_units.append({
                **unit,
                ...
                'planned_aligned_wav_path': str(OUT_DIR / f"heresy-between-render-unit-openutau-v12-{unit['id']}.wav"),
        ...
        ustx_path = OUT_DIR / f"heresy-between-render-unit-openutau-v12-{unit['id']}.ustx"
        if ustx_path.exists():
    ...
    manifest = {
        ...
        'kind': 'render-unit OpenUTAU v12 full timeline',
        ...
        'note_strategy': 'v12 promotes v11 audited phrase probes into generated render units: exact assembly spans, optional render context, render-tail extension for short phrases, small-tsu preservation, long-vowel preservation for segment 11, and stable pitch for segments 10-36.',
    }
